...
async def verify_license() -> bool:
    """验证许可证"""
    try:
        key = input("请输入您的卡密: ").strip()
        if not key:
            LOG.error("卡密不能为空")
            return False
...
cd /www/wwwroot/verify
pwd        r = await CLIENT.get(url, headers=headers)
        r_json = r.json()
...
if __name__ == "__main__":
    try:
        init()
        if not asyncio.run(verify_license()):
            LOG.error("验证失败，程序退出")
            os.system("pause")
            sys.exit(1)
